...
#'
#' @examples
#' check_na(5,7,NA) # triggers NA warning and removes NA
#' try(check_na(NA,NA)) # throws an error
check_na <- function (x) {
  if (any(is.na(x))) {
...
#'
#' @examples
#' check_numeric(5,5,6,7) # returns c(5,5,6,7)
#' try(check_numeric(3,4,5,M,N)) # throws an error
#' try(check_numeric(Inf,-Inf) # throws an error
#' try(check_numeric(TRUE,TRUE,FALSE) # throws an error and avoids R conversion to 0 and 1
...
#'
#' @examples
#' check_empty(12,4,5,6,7) # returns c(12,4,5,6,7)
...
#'
#' @examples
#' check_single_value(23,44,5,68)
#' try(check_single_value(5)) # displays a warning
...
#'
#' @examples
#' validate_vector(5,4,55,88,23, NA, 32) # returns c(5,4,55,88,23,32)
#' try(validate_vector(NA, "c", 6,2,3,1)) #throws error
